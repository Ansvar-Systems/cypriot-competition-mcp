# ─────────────────────────────────────────────────────────────────────────────
# Cypriot Competition (CPC) MCP — multi-stage Dockerfile
# ─────────────────────────────────────────────────────────────────────────────
# Build:  docker build -t cypriot-competition-mcp .
# Run:    docker run --rm -p 3000:3000 cypriot-competition-mcp
#
# The image bakes /app/data/cpc-cy.db at build time. Override CPCCY_DB_PATH
...
ENV CPCCY_DB_PATH=/app/data/cpc-cy.db
...
# Bake the pre-built database into the image so /app/data/cpc-cy.db resolves
...
# We then COPY it into the image at /app/data/cpc-cy.db (CPCCY_DB_PATH). The
# explicit `data/<name>.db` reference is required for the workflow's grep
# `COPY\s+\K(data/\S+\.db)` to match.
COPY data/database.db data/cpc-cy.db
